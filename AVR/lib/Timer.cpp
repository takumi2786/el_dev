...
Timer::Timer(int division,int OCR1A_, int OCR1B_){
...
void Timer::set_outputA(int mode){
    //0:標準ポート動作 (OC1x切断)
...
void Timer::set_outputB(int mode){
    //0:標準ポート動作 (OC1x切断)
    //1:比較一致でOC1Aピン トグル(交互)出力、OC1Bは標準ポート動作(OC1B切断)
...
void Timer::set_division(int division){
    // 分周の設定
...
// タイマー割り込みの有効化
void Timer::set_interrupt(int interrupt_type){
