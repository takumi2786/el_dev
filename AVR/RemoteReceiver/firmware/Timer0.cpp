...
void Timer0::set_interrupt(int interrupt_type){
    ...
    case 0:
        this->interrupt_period=0.125*division*OCR0A;//高速PWM以外だとTOP値が変わるので注意
        TIMSK0 |= (0<< TOIE0);//TOIE1割り込みを利用>>TIMER1_OVF_vect
        break;
    case 1:
        this->interrupt_period=0.125*division*OCR0A;
        TIMSK0 |= (1<< OCIE0A);//OCIE1A割り込みを利用>>TIMER1_COMPA_vect
        break;
    case 2:
        this->interrupt_period=0.125*division*OCR0B;
...
void Timer0::set_OCRA(int T){
    OCR0A = T;
}

void Timer0::set_OCRB(int T){
    OCR0B = T;
}